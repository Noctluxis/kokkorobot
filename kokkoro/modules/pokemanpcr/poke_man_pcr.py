...
import kokkoro
from kokkoro import Service, util, Bot
from kokkoro.util import FreqLimiter, ecoop
from kokkoro.modules.priconne.pcr_data import chara, _pcr_data
from kokkoro.typing import MessageSegment, NotifyEvent, GroupMessageEvent, T_State, MessageEvent
from kokkoro.ecoopshop import collectshoplimit
from .utils import *
...
DIR_PATH = os.path.join(os.path.expanduser(
    kokkoro.configs.RES_DIR), 'img', 'priconne', 'unit')
DB_PATH = os.path.expanduser("./data/poke_man_pcr.db")
POKE_GET_CARDS = 0.75           # 每一戳的卡片掉落几率
POKE_DAILY_LIMIT = 0            # 机器人每天掉落卡片的次数
...
RESET_HOUR = 5                  # 每日戳一戳、赠送等指令使用次数的重置时间，0代表凌晨0点，1代表凌晨1点，以此类推
...
poke_notice = sv.on_notice(only_group=True, block=True)


@poke_notice.handle()
async def poke_back(bot: Bot, event: NotifyEvent):
    if event.sub_type != 'poke':
        return
    uid = event.user_id
    at_user = MessageSegment.at(event.user_id)
    guid = event.group_id, event.user_id
    if not cooling_time_limiter.check(uid):
        return
    cooling_time_limiter.start_cd(uid)
    if event.target_id != event.self_id:
        return
    sum_lmt = lmt_num(uid) + collectshoplimit.get_num(uid)
    if not daily_limiter.check(guid, sum_lmt) and not daily_tip_limiter.check(guid):
        poke_tip_cd_limiter.start_cd(guid)
    if not daily_limiter.check(guid, sum_lmt) and poke_tip_cd_limiter.check(guid):
        daily_tip_limiter.increase(guid)
        await poke_notice.finish(f'你今天戳得已经够多的啦（{sum_lmt}），再戳也不会有奇怪的东西掉下来的~', at_sender=True)
    daily_tip_limiter.reset(guid)
    if not daily_limiter.check(guid, sum_lmt) or random.random() > POKE_GET_CARDS:
        if random.random() > 0.7:
            ...
            msg = f'你今天戳得已经够多的啦（{sum_lmt}），再戳也不会有奇怪的东西掉下来的~'
        ...
        card_counter, card_descs, card = get_random_cards(db.get_cards_num(event.group_id, event.user_id), row_num, col_num,
                                                          amount, True, get_random_cards_list, SUPER_RARE_PROBABILITY, RARE_PROBABILITY)
        dash = '----------------------------------------'
        msg_part = '\n'.join(card_descs)
        await poke_notice.send(f'别戳了别戳了o(╥﹏╥)o' + card + at_user +
                               f'这些卡送给你了, 让我安静会...\n{dash}\n获得了:\n{msg_part}')
        for card_id in card_counter.keys():
            db.add_card_num(event.group_id, event.user_id, card_id, card_counter[card_id])
        daily_limiter.increase(guid)
        return

mix = sv.on_fullmatch('合成', aliases={'献祭', '融合'})


@mix.handle()
async def mix_card(bot: Bot, event: GroupMessageEvent, state: T_State):
    # 参数识别
    s = event.message.extract_plain_text()
    args = s.split(' ')
    if len(args) != 2:
        await mix.finish('请输入想要合成的两张卡, 以空格分隔')
    card1_id = get_card_id_by_card_name(args[0])
    card2_id = get_card_id_by_card_name(args[1])
    if not card1_id:
        await mix.finish(f'错误: 无法识别{args[0]}, 若为稀有或超稀有卡请在名称前加上"稀有"或"超稀有"')
    if not card2_id:
        await mix.finish(f'错误: 无法识别{args[1]}, 若为稀有或超稀有卡请在名称前加上"稀有"或"超稀有"')
    card1_num = db.get_card_num(event.group_id, event.user_id, card1_id)
    card2_num = db.get_card_num(event.group_id, event.user_id, card2_id)
    if card1_id == card2_id:
        if card1_num < 2:
            await mix.finish(f'{get_card_name_with_rarity(args[0])}卡数量不足, 无法合成')
    else:
        if card1_num == 0:
            await mix.finish(f'{get_card_name_with_rarity(args[0])}卡数量不足, 无法合成')
        if card2_num == 0:
            await mix.finish(f'{get_card_name_with_rarity(args[1])}卡数量不足, 无法合成')
    # 开始献祭
    ...
    card_counter, card_descs, card = get_random_cards(db.get_cards_num(
        event.group_id, event.user_id), 1, 1, 1, False, get_random_cards_list, super_rare_prob, rare_prob)
    card_id = list(card_counter.keys())[0]
    rarity_desc, chara_name = get_chara_name(card_id)
    db.add_card_num(event.group_id, event.user_id, card1_id, -1)
    db.add_card_num(event.group_id, event.user_id, card2_id, -1)
    db.add_card_num(event.group_id, event.user_id, card_id)
    await mix.finish(f'将两张卡片进行了融合……然后' + card +
                     f'获得了{rarity_desc}「{chara_name}」×1~', at_sender=True)
auto_mix = sv.on_fullmatch('一键合成', aliases={'一键献祭', '一键融合', '全部献祭', '全部合成', '全部融合'})


@auto_mix.handle()
async def auto_mix_card(bot: Bot, event: GroupMessageEvent, state: T_State):
    # 参数识别
    s = event.message.extract_plain_text()
    args = s.split(' ')
    if len(args) == 2 and args[0] in rarity_desc2rarity and args[1] in rarity_desc2rarity:
        ...
        await auto_mix.finish('参数格式错误, 请按正确格式输入指令参数')
    # 自动消耗多余的卡
    surplus_cards = db.get_surplus_cards(event.group_id, event.user_id)
    ...
    if args[0] == args[1]:
        ...
        if len(args) == 3 and int(args[2])*2 > available_card_amount:
            await auto_mix.finish(f'合成失败, 多余的【{args[0]}】卡数量不足{args[2]*2}, 无法一键合成{args[2]}次.')
        if len(args) == 2 and available_card_amount < 2:
            await auto_mix.finish(f'合成失败, 多余的【{args[0]}】卡数量不足2, 无法一键合成')
        ...
        for card_id in available_cards:
            ...
            if mixed_cards_amount + card_amount <= 2 * mix_rounds:
                db.add_card_num(event.group_id, event.user_id, card_id, -card_amount)
                mixed_cards_amount += card_amount
            else:
                db.add_card_num(event.group_id, event.user_id,
        ...
        if len(args) == 3:
            if int(args[2]) > available_card_amount1:
                await auto_mix.finish(f'合成失败, 多余的【{args[0]}】卡数量不足{args[2]}, 无法一键合成{args[2]}次.')
            if int(args[2]) > available_card_amount2:
                await auto_mix.finish(f'合成失败, 多余的【{args[1]}】卡数量不足{args[2]}, 无法一键合成{args[2]}次.')
        if len(args) == 2:
            if available_card_amount1 < 1:
                await auto_mix.finish(f'合成失败, 多余的【{args[0]}】卡数量不足1, 无法一键合成')
            if available_card_amount2 < 1:
                await auto_mix.finish(f'合成失败, 多余的【{args[1]}】卡数量不足1, 无法一键合成')
    ...
    card_counter, card_descs, card = get_random_cards(db.get_cards_num(event.group_id, event.user_id),
                                                      row_num, col_num, mix_rounds, False,
                                                      get_random_cards_list, super_rare_prob, rare_prob)
    msg_part = '\n'.join(card_descs)
    await auto_mix.send(f'进行了{mix_rounds}轮融合……然后' + card +
                        f'获得了:\n{msg_part}', at_sender=True)
    for card_id in card_counter.keys():
        db.add_card_num(event.group_id, event.user_id,
                        card_id, card_counter[card_id])

exchange = sv.on_fullmatch('交换', aliases={'交易', '互换'})


@exchange.handle()
async def exchange_cards(bot: Bot, event: GroupMessageEvent, state: T_State):
    # 参数识别
    if len(event.message) != 3:
        await exchange.finish('参数格式错误, 请重试')
    if event.message[0].type != 'text' or event.message[1].type != 'at' or event.message[2].type != 'text':
        await exchange.finish('参数格式错误, 请重试')
    target_uid = int(event.message[1].data['qq'])
    if event.user_id == target_uid:
        await exchange.finish('不能和自己交换卡片哦')
    card1_name = event.message[0].data['text'].strip()
    card2_name = event.message[2].data['text'].strip()
    card1_id = get_card_id_by_card_name(card1_name)
    card2_id = get_card_id_by_card_name(card2_name)
    if not card1_id:
        await exchange.finish(f'错误: 无法识别{get_card_name_with_rarity(card1_name)}, 若为稀有或超稀有卡请在名称前加上"稀有"或"超稀有"')
    if not card2_id:
        await exchange.finish(f'错误: 无法识别{get_card_name_with_rarity(card2_name)}, 若为稀有或超稀有卡请在名称前加上"稀有"或"超稀有"')
    if card1_id == card2_id:
        await exchange.finish(f'不能交换相同的卡片哦')
    card1_num = db.get_card_num(event.group_id, event.user_id, card1_id)
    card2_num = db.get_card_num(event.group_id, target_uid, card2_id)
    if card1_num == 0:
        await exchange.finish(MessageSegment.at(event.user_id) +
                              f'的{get_card_name_with_rarity(card1_name)}卡数量不足, 无法交换')
    if card2_num == 0:
        await exchange.finish(MessageSegment.at(target_uid) +
                              f'的{get_card_name_with_rarity(card2_name)}卡数量不足, 无法交换')
    # 发起交换请求
    if exchange_request_master.has_exchange_request_to_confirm(event.group_id, target_uid):
        await exchange.finish('您发起交易的对象目前正与他人交易中, 请稍等~', at_sender=True)
    exchange_request_master.add_exchange_request(event.group_id, target_uid, ExchangeRequest(
        event.user_id, card1_id, card1_name, target_uid, card2_id, card2_name))
    await exchange.send(MessageSegment.at(target_uid) +
                        f'\n叮~' + MessageSegment.at(event.user_id) +
                        f'希望用他的{get_card_name_with_rarity(card1_name)}卡交换'
                        f'你的{get_card_name_with_rarity(card2_name)}卡，输入"/确认交换"可完成交换'
                        f'({REQUEST_VALID_TIME}s后交换请求失效)')

confirm = sv.on_fullmatch('确认交换', aliases={'同意交换'})


@confirm.handle()
async def confirm_exchange(bot: Bot, event: GroupMessageEvent):
    if not exchange_request_master.has_exchange_request_to_confirm(event.group_id, event.user_id):
        await confirm.finish('您还没有收到换卡请求~', at_sender=True)
    exchange_request = exchange_request_master.get_exchange_request(
        event.group_id, event.user_id)
    exchange_request_master.delete_exchange_request(event.group_id, event.user_id)
    card1_num = db.get_card_num(
        event.group_id, exchange_request.sender_uid, exchange_request.card1_id)
    card2_num = db.get_card_num(
        event.group_id, exchange_request.target_uid, exchange_request.card2_id)
    if card1_num == 0:
        await confirm.finish(f'{MessageSegment.at(exchange_request.sender_uid)}'
                             f'的{get_card_name_with_rarity(exchange_request.card1_name)}卡数量不足, 无法交换')
    if card2_num == 0:
        await confirm.finish(f'{MessageSegment.at(exchange_request.target_uid)}'
                             f'的{get_card_name_with_rarity(exchange_request.card2_name)}卡数量不足, 无法交换')
    db.add_card_num(event.group_id, exchange_request.sender_uid,
                    exchange_request.card1_id, -1)
    db.add_card_num(event.group_id, exchange_request.target_uid,
                    exchange_request.card2_id, -1)
    db.add_card_num(event.group_id, exchange_request.sender_uid,
                    exchange_request.card2_id)
    db.add_card_num(event.group_id, exchange_request.target_uid,
                    exchange_request.card1_id)
    await confirm.finish('交换成功!')

give = sv.on_fullmatch('赠送', aliases={'白给', '白送'})


@give.handle()
async def give_handle(bot: Bot, event: GroupMessageEvent, state: T_State):
    if len(event.message) != 2 or event.message[0].type != 'at' or event.message[1].type != 'text':
        await give.finish('参数格式错误, 请重试')
    target_uid = int(event.message[0].data['qq'])
    if not daily_give_limiter.check((event.group_id, target_uid)):
        await give.finish(MessageSegment.at(target_uid) + f'的今日接受赠送次数已达上限，明天再送给TA吧~')
    if target_uid == event.user_id:
        await give.finish('不用给自己赠卡~')
    card_name = event.message[1].data['text'].strip()
    card_id = get_card_id_by_card_name(card_name)
    if not card_id:
        await give.finish(f'错误: 无法识别{get_card_name_with_rarity(card_name)}, 若为稀有或超稀有卡请在名称前加上"稀有"或"超稀有"')
    card_num = db.get_card_num(event.group_id, event.user_id, card_id)
    if card_num < 1:
        await give.finish(f'{get_card_name_with_rarity(card_name)}卡数量不足, 无法赠送')
    db.add_card_num(event.group_id, event.user_id, card_id, -1)
    db.add_card_num(event.group_id, target_uid, card_id)
    daily_give_limiter.increase((event.group_id, target_uid))
    await give.finish(MessageSegment.at(event.user_id) +
                      f'将{get_card_name_with_rarity(card_name)}赠送给了' + MessageSegment.at(target_uid))

storage = sv.on_fullmatch('查看仓库')


@storage.handle()
async def storage_handle(bot: Bot, event: GroupMessageEvent):
    if len(event.message) == 0:
        uid = event.user_id
    elif event.message[0].type == 'at':
        uid = int(event.message[0].data['qq'])
    else:
        await storage.finish('参数格式错误, 请重试')
    row_nums = {}
    ...
    cards_num = db.get_cards_num(event.group_id, uid)
    ...
    for star in cards.keys():
        ...
        row_offset += 30
    ranking = db.get_group_ranking(event.group_id, uid)
    ...
    await storage.finish(MessageSegment.at(uid) + f'的仓库:' + MessageSegment.image(file=base64_str) +
                         f'\n持有卡片数: {total_card_num}\n'
                         f'普通卡收集: {normalize_digit_format(normal_card_num)}/'
                         f'{normalize_digit_format(normal_card_total)}\n'
                         f'稀有卡收集: {normalize_digit_format(rare_card_num)}/'
                         f'{normalize_digit_format(rare_card_total)}\n'
                         f'超稀有收集: {normalize_digit_format(super_rare_card_num)}/'
                         f'{normalize_digit_format(super_rare_card_total)}\n'
                         f'图鉴完成度: {normalize_digit_format(len(cards_num))}/'
                         f'{normalize_digit_format(len(card_file_names_all))}\n'
                         f'当前群排名: {ranking_desc}')

refresh = sv.on_fullmatch('刷新卡片')


# 当增加新角色后不重启kokkoro刷新现有缓存
@refresh.handle()
async def refresh_unit_cache(bot: Bot, event: MessageEvent):
    if event.user_id not in kokkoro.configs.SUPERUSERS:
        await refresh.finish('Insufficient authority.')
