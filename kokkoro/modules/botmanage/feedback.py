import kokkoro
from kokkoro import Bot, Service, priv
from kokkoro.typing import (CQEvent, GroupMessageEvent, Message,
...
async def feedback(bot: Bot, event: CQEvent, state: T_State):
    ...
    user_info = await bot.get_stranger_info(user_id=uid)
    nickname = user_info.get('nickname', '未知用户')
    su = kokkoro.configs.SUPERUSERS[0]
    if isinstance(event, GroupMessageEvent):
        await bot.send_private_msg(user_id=su, message=f'来自群聊{event.group_id}的{nickname}({uid})：\n========\n{text}')
        await bot.send(event, Message(f'>{nickname}\n您的反馈已发送至维护组！\n========\n{text}'))
    elif isinstance(event, PrivateMessageEvent):
        await bot.send_private_msg(user_id=su, message=f'{nickname}({uid})：\n========\n{text}')
