...
import kokkoro
from kokkoro import configs, Bot
...
async def disconnect(bot: Bot):
    ...
    try:
        ...
        kokkoro.logger.error("WebSocket连接已断开")
...
async def kick_me_alert(bot: Bot, event: GroupDecreaseNoticeEvent):
    if event.is_tome():
        ...
        if operator_id != event.self_id:    # ignore myself
            ...
            await bot.send_private_msg(user_id=feed_back, message=f'已被用户{nickname}({operator_id})移出群聊{group_id}')
...
async def ban_me_alert(bot: Bot, event: GroupBanNoticeEvent):
    ...
    if event.duration:
        ...
        user_info = await bot.get_stranger_info(user_id=operator_id)
        nickname = user_info.get('nickname', '未知用户')
        await bot.send_private_msg(user_id=feed_back, message=f'已被群聊{group_id}的用户{nickname}({operator_id})禁言{event.duration}秒')
    else:
        group_id = event.group_id
        operator_id = event.operator_id
        feed_back = configs.SUPERUSERS[0]
        user_info = await bot.get_stranger_info(user_id=operator_id)
        nickname = user_info.get('nickname', '未知用户')
        await bot.send_private_msg(user_id=feed_back, message=f'已被解除群聊{group_id}的用户{nickname}({operator_id})的禁言')