...
import kokkoro
from kokkoro import Service, priv, util
from kokkoro.configs import twitter as cfg
from kokkoro.typing import MessageSegment as ms
try:
...
class UserIdCache:
    _cache_file = os.path.expanduser('./data/twitter_uid_cache.json')
    ...
    async with client:
        while True:
            try:
                ...
                kokkoro.logger.error(f"Error {type(e)} Occurred in twitter stream. Restarting stream in 5s.")
...
async def open_stream(client: PeonyClient):
    ...
    kokkoro.logger.info(f"订阅推主={router.follows.keys()}, {follow_ids=}")
    stream = client.stream.statuses.filter.post(follow=follow_ids)
    async with stream:
        async for tweet in stream:
            kokkoro.logger.debug("Got twitter event.")
            if peony.events.tweet(tweet):
                ...
                kokkoro.logger.info(f"推送推文：\n{msg}")
                for s in entry.services:
                    await s.broadcast(msg, f' @{screen_name} 推文', 0.3)
            else:
                kokkoro.logger.debug("Ignore non-tweet event.")
