...
import aiohttp
import kokkoro
from kokkoro import Bot, Service
from kokkoro.typing import CQEvent, MessageEvent, ActionFailed, T_State, GroupMessageEvent, PrivateMessageEvent
try:
...
sv = Service('机器翻译', bundle='通用', help_=TXT)

app_id = kokkoro.configs.translate.tencent_app_id
app_key = kokkoro.configs.translate.tencent_app_key
...
async def _(bot: Bot, event: CQEvent, state: T_State):
    if isinstance(event, MessageEvent):
        ...
        kokkoro.logger.warning('Not supported: translator')
        return
