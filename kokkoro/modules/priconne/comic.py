...
try:
    ...
    import json
import kokkoro
from kokkoro import aiohttpx, R, Service, Bot, scheduler
from kokkoro.typing import CQEvent, T_State, FinishedException, Message, GroupMessageEvent, PrivateMessageEvent
...
async def download_img(save_path, link):
    ...
    会覆盖原有文件，需保证目录存在
    '''
    kokkoro.logger.info(f'download_img from {link}')
    resp = await aiohttpx.get(link, timeout=(3.05, 6.05))
    kokkoro.logger.info(f'status_code={resp.status_code}')
    if 200 == resp.status_code:
        if re.search(r'image', resp.headers['content-type'], re.I):
            kokkoro.logger.info(f'is image, saving to {save_path}')
            with open(save_path, 'wb') as f:
                f.write(resp.content)
                kokkoro.logger.info('saved!')
...
async def download_comic(id_):
    ...
    kokkoro.logger.info(f'getting comic {id_} ...')
    url = base + id_
    kokkoro.logger.info(f'url={url}')
    ...
    kokkoro.logger.info(f'episode={index[episode]}')
    # 下载图片并保存
...
async def update_seeker():
    ...
    index = load_index()
    # 获取最新漫画信息
    resp = await aiohttpx.get(index_api, timeout=(3.05, 6.05))
    data = resp.json()
    ...
    if episode in index:
        ...
        if id_ == old_id:
            kokkoro.logger.info('未检测到官漫更新')
            return
    # 确定已有更新，下载图片
    kokkoro.logger.info(f'发现更新 id={id_}')
