...
import kokkoro
from kokkoro import Service, Bot, priv, util, log
from kokkoro.util import DailyNumberLimiter, concat_pic, pic2b64, ecoop
from kokkoro.typing import CQEvent, Message, MessageSegment, T_State, GroupMessageEvent, PrivateMessageEvent, FinishedException
from kokkoro.service import parse_uid
from kokkoro.modules.priconne.pcr_data import chara
from kokkoro.modules.priconne.pool import Gacha
from kokkoro.ecoopshop import jewelshoplimit, tenjoshoplimit
try:
...
tenjo_limit = DailyNumberLimiter()

POOL = ('MIX', 'JP', 'TW', 'BL')
DEFAULT_POOL = POOL[0]

_pool_config_file = os.path.expanduser('./data/group_pool_config.json')
_group_pool = {}
try:
...
async def check_jewel_num(bot: Bot, event: CQEvent):
    ...
    nickname = user_info.get('nickname', '未知用户')
    jewel_sum_lmt = jewel_lmt_num(uid) + jewelshoplimit.get_num(uid)
    JEWEL_EXCEED_NOTICE = f'您今天已经抽过{jewel_sum_lmt}钻了，欢迎明早5点后再来！'
    if not jewel_limit.check(uid, jewel_sum_lmt):
        if isinstance(event, GroupMessageEvent):
            sender = f'>{nickname}\n'
...
async def check_tenjo_num(bot: Bot, event: CQEvent):
    ...
    user_info = await bot.get_stranger_info(user_id=uid)
    nickname = user_info.get('nickname', '未知用户')
    tenjo_sum_lmt = tenjo_lmt_num(uid) + tenjoshoplimit.get_num(uid)
    TENJO_EXCEED_NOTICE = f'您今天已经抽过{tenjo_sum_lmt}张天井券了，欢迎明早5点后再来！'
    if not tenjo_limit.check(uid, tenjo_sum_lmt):
        if isinstance(event, GroupMessageEvent):
            sender = f'>{nickname}\n'
...
def tenjo_lmt_num(uid):
    coop_level = ecoop.get_coop_level(uid)
    return 1 if coop_level < 5 else 2
