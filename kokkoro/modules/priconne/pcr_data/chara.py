...
import kokkoro
from kokkoro import R, util, Bot
from kokkoro.typing import CQEvent, FinishedException
from kokkoro.modules.priconne.pcr_data import _pcr_data
...
class Roster:
    ...
    def update(self):
        _pcr_data.chara_master.load_pcr_data()
        self._roster.clear()
        for idx, names in _pcr_data.CHARA_NAME.items():
            for n in names:
                ...
                if n not in self._roster:
                    ...
                    kokkoro.logger.warning(f'priconne.chara.Roster: 出现重名{n}于id{idx}与id{self._roster[n]}')
...
def download_chara_icon(id_, star):
    ...
    kokkoro.logger.info(f'Downloading chara icon from {url}')
    try:
        rsp = requests.get(url, stream=True, timeout=5)
    except Exception as e:
        kokkoro.logger.error(f'Failed to download {url}. {type(e)}')
        kokkoro.logger.exception(e)
    if 200 == rsp.status_code:
        img = Image.open(BytesIO(rsp.content))
        img.save(save_path)
        kokkoro.logger.info(f'Saved to {save_path}')
    else:
        kokkoro.logger.error(f'Failed to download {url}. HTTP {rsp.status_code}')
...
class Chara:
    ...
    def render_icon(self, size, star_slot_verbose=True) -> Image:
        try:
            ...
            kokkoro.logger.error(f'File not found: {self.icon.path}')
