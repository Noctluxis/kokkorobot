...
import kokkoro
from kokkoro import aiohttpx, configs, Service, R, Bot
from kokkoro.typing import CQEvent, MessageSegment, GroupMessageEvent, PrivateMessageEvent, FinishedException, T_State
from kokkoro.util import FreqLimiter, pic2b64
...
"""
DB_PATH = os.path.expanduser("./data/arena_db.json")
DB = {}
try:
    ...
    kokkoro.logger.warning(f"arena_db.json not found, will create when needed.")
...
async def do_query(id_list, user_id, region=1):
    ...
    kokkoro.logger.debug(f"Arena query {payload=}")
    try:
        ...
        res = resp.json
        kokkoro.logger.debug(f"len(res)={len(res)}")
    except Exception as e:
        kokkoro.logger.exception(e)
        return None
    if res["code"]:
        kokkoro.logger.error(f"Arena query failed.\nResponse={res}\nPayload={payload}")
...
async def _arena_query(bot: Bot, event: CQEvent, state: T_State, region: int):
    ...
    if 1004 in defen:
        ...
        raise FinishedException
    # 执行查询
    kokkoro.logger.info('Doing query...')
    res = None
    try:
        res = await do_query(defen, uid, region)
    except kokkoro.aiohttpx.HTTPError as e:
        ...
        raise FinishedException
    kokkoro.logger.info('Got response!')
    # 处理查询结果
    if res is None:
    ...
    # 发送回复
    kokkoro.logger.info('Arena generating picture...')
    teams = render_atk_def_teams(res)
    teams = pic2b64(teams)
    teams = MessageSegment.image(teams)
    kokkoro.logger.info('Arena picture ready!')
    # 纯文字版
    ...
    kokkoro.logger.debug('Arena sending result...')
    await bot.send(event, '\n'.join(msg))
    kokkoro.logger.debug('Arena result sent!')
    raise FinishedException