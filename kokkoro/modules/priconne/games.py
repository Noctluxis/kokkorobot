...
import asyncio
import random
from kokkoro import Service, Bot, util
from kokkoro.typing import MessageSegment, Message, CQEvent, GroupMessageEvent, PrivateMessageEvent
from kokkoro.modules.priconne.pcr_data import _pcr_data, chara
...
D_TURN_TIME = 12
TURN_NUMBER = 5
DB_PATH_AVATAR = os.path.expanduser("./data/pcr_avatar_guess.db")
DB_PATH_DESC = os.path.expanduser("./data/pcr_desc_guess.db")
...
async def a_input_chara_name(bot: Bot, event: CQEvent):
    ...
    if c.id != chara.UNKNOWN and c.id == game1.answer:
        ...
        n = game1.record()
        ecoop = Ecoop(event.user_id)
        eco_random = random.randint(4, 6)
        ecoop.add_ecoop('eco', ecoop=eco_random, reason='game win')
        msg = f"正确答案是：{c.name}{c.icon.cqcode}\n{MessageSegment.at(event.user_id)}猜对了，真厉害!" \
              f"恭喜获得{eco_random}金币!TA已经猜对{n}次了~\n(此轮游戏将在几秒后自动结束，请耐心等待)"
        await bot.send(event, Message(msg))


@svd.on_message(priority=2)
