...
import kokkoro
from kokkoro import aiohttpx, Service, scheduler, Bot
from kokkoro.typing import List, Union, CQEvent


svtw = Service('pcr-news-tw', bundle='pcr订阅', help_='台服新闻', enable_on_default=False)
svbl = Service('pcr-news-bili', bundle='pcr订阅', help_='B服新闻', enable_on_default=False)
...
class BaseSpider(abc.ABC):
    ...
    async def get_response(cls) -> aiohttpx.Response:
        resp = await aiohttpx.get(cls.url, follow_redirects=True)
...
async def news_poller(spider: BaseSpider, sv: Service, TAG):
    if not spider.item_cache:
        ...
        kokkoro.logger.info(f'{TAG}新闻缓存为空，已加载至最新')
        return
    news = await spider.get_update()
    if not news:
        kokkoro.logger.info(f'未检索到{TAG}新闻更新')
        return
    kokkoro.logger.info(f'检索到{len(news)}条{TAG}新闻更新！')
