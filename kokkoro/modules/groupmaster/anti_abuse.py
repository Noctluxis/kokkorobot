...
from kokkoro import Bot, R, util, priv
import kokkoro
from kokkoro.typing import CQEvent, GroupMessageEvent, PrivateMessageEvent, Message
...
anti_crit = on_keyword(BANNED_WORD, to_me())
...
async def ban_word(bot: Bot, event: CQEvent):
    ...
    kokkoro.logger.critical(f'Self: {event.self_id}, Message {event.message_id} from {user_id}: {event.message}')
