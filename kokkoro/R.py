...
class ResObj:
    def __init__(self, res_path):
        res_dir = os.path.expanduser(kokkoro.configs.RES_DIR)
    ...
    def url(self):
        ...
        return urljoin(kokkoro.configs.RES_URL, pathname2url(self.__path))

    @property
    def path(self):
        """资源文件的路径，供bot内部使用"""
        return os.path.join(kokkoro.configs.RES_DIR, self.__path)
...
class ResImg(ResObj):
    ...
    def cqcode(self) -> MessageSegment:
        if kokkoro.configs.RES_PROTOCOL == 'http':
            return MessageSegment.image(self.url)
        elif kokkoro.configs.RES_PROTOCOL == 'file':
