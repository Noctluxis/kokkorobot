...
import kokkoro
from kokkoro import priv, Bot
from kokkoro.util import normalize_str, construct_node_message
from kokkoro.typing import *
from kokkoro.log import wrap_logger
try:
...
_service_config_dir = os.path.expanduser('./data/service_config/')
...
class Service:
    ...
    async def get_enable_groups(self) -> Dict[int, List[Bot]]:
        gl = defaultdict(list)
        for bot in kokkoro.get_bot_list():
    ...
    async def broadcast(self, msgs: Optional[Iterable], tag='', interval_time=0.5, node_msg: bool = False):
        ...
        for gid in gdict.keys():
            for bot in gdict[gid]:
                ...
                if not node_msg:
                    ...
                    node_msgs = construct_node_message(custom_user_id=int(bot.self_id), msg_list=msgs)
                    try:
                        msgs.insert(2, '图片过多，其余图片请于转发消息中查看')
                        for msg in msgs[0:3]:
                            await asyncio.sleep(interval_time)
                            try:
                                await bot.send_group_msg(group_id=gid, message=msg)
                                ...
                                self.logger.error(f'{sid}在群{gid}投递{tag}失败')
                        await asyncio.sleep(interval_time)
                        await bot.send_group_forward_msg(group_id=gid, messages=node_msgs)
                        self.logger.info(f"{sid}在群{gid}投递{tag}成功")
                    except:
                        self.logger.error(f'{sid}在群{gid}投递{tag}失败')
